...
int main() {
	...
	string write_location1 = "G:\\wiki\\dataset\\"; // writeFile_위치1 
	...
	for (int fold = 0; fold <= AG; fold++) {
		...
		for (int i = 0; i < 100; i++) {
			...
			file_name = read_location1 + location2 + location3 + location4 + location5; //파일을 읽기 위한 파일 이름 설정 
			wifstream in(file_name);
			in.imbue(locale(locale::empty(), new codecvt_utf8<wchar_t, 0x10ffff, consume_header>)); //파일을 utf-8형태로 사용
			for (k = 0; k < 30000; k++) {
			...
			for (j = 0; j <= k; j++) {
				for (j; j <= k; j++) {
					...
					if (str[j].length() == 0) continue; //공백라인이면 length==0이므로 continue
					if (str[j].back() != '.') continue; //만약 .로 끝나지 않으면 무시하고 continue
