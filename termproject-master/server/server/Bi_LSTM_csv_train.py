...
for i in line:
    ...
    sentence = okt.pos(i[0], norm=True, stem=True)
    temp = []
    temp_embedding = []
    all_temp = []
    for k in range(len(sentence)):
        temp_embedding.append(sentence[k][0]) #형태소 추가
        temp.append(sentence[k][0] + '/' + sentence[k][1]) #형태소/형태소 종류
    all_temp.append(temp) #[형태소/형태소 종류,형태소/형태소 종류,...]
    embeddingmodel.append(temp_embedding) #형태소
    ...
    all_temp.append(category_number_dic.get(category))#뉴스 종류
